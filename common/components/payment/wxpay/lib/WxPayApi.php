 ...
 * WxPayApi.php
 * @author: allen
 * @date  2020年4月29日下午2:50:35
 * @copyright  Copyright igkcms
 */
namespace common\components\payment\wxpay\lib;
/**
 * 
...
class WxPayApi
{
	...
	public static function unifiedOrder($config, $inputObj, $timeOut = 6)
	{
		...
		if($inputObj->GetTrade_type() == "MWEB" && !$inputObj->IsScene_info()){
		    throw new Exception("统一支付接口中，缺少必填参数scene_info！trade_type为MWEB时，scene_info为必填参数！");
		}
		
		//异步通知url未设置，则使用配置文件中的url
		if(!$inputObj->IsNotify_urlSet() && $config['notify_url'] != ""){
	...
	public static function shorturl($config, $inputObj, $timeOut = 6)
	{
 	 ...
 	 */
	public static function notify($config, &$msg)
	{
		//获取通知的数据
